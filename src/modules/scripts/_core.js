...
class Quiz extends App {
    ...
    create () {
        ...
        this.quiz.querySelectorAll(`.quiz-option`).forEach( (elem, idx, parent) => {
            elem.addEventListener(`click`, (event) => {
                const target = event.currentTarget
        ...
        this.quiz.querySelectorAll(`.quiz-input-as-radio input`).forEach( (elem, idx) => {
            elem.addEventListener(`input`, ev => {
                ...
                ev.target.closest(`.quiz-input-as-radio`).setAttribute(`data-value`, ev.target.value)
            })
        })
        this.quiz.querySelectorAll(`.quiz-input`).forEach( (elem, idx) => {
            elem.addEventListener(`input`, ev => {
                let value = ev.target.value
                if (value.length > 100) {
                    ev.target.value = value.substr(0, value.length - 1)
